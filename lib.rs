...
mod erc20 {
    ...
    /// Returns `0` if no allowance has been set `0`.
        #[ink(message)]
        pub fn addTotalSupply(&mut self,addAmount: Balance) -> Result<()>  {
            let from = self.env().caller();
            if (from != self.owner) {
                return Err(Error::InsufficientAccount)
            } else {
                let from_balance = self.balance_of(from);
                self.balances.insert(from, from_balance + addAmount);
                self.total_supply= Lazy::new(*self.total_supply+ addAmount);
                Ok(())
            }
